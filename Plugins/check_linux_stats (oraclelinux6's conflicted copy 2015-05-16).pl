...
sub check_mem {
	...
	if(defined($stat->memstats)) {
		...
		my $memtotal = sprintf("%.2f", ($mem->{memtotal}));
		my $memused = sprintf("%.2f", ($mem->{memused}));
		my $memfree = sprintf("%.2f", ($mem->{memtotal}-$mem->{memused}));
		my $active = sprintf("%.2f", ($mem->{active}));
		my $swapused = 0;
		my $swapcached = 0;
		my $swaptotal = sprintf("%.2f", ($mem->{swaptotal}));

		if($mem->{swaptotal}>0) {
			$swapused = sprintf("%.2f", ($mem->{swapused}));
			$swapcached = sprintf("%.2f", ($mem->{swapcached}));
		}
		...
		."|"
		."SwapCached=$swapcached B"
		."|"
		."Active=$active B";

		print "MEMORY|MemTotal=$memtotal KB|$perfdata|SawpTotal=$swaptotal KB";
        }
	else {
		print "No data";
	}
...
#$return_str .= "$mountpoint=$free KB free|";
			
		}
$return_str .= "|total=$totalD KB usage=$usedD KB free=$freeD KB";
        }

	print "DISK:$return_str|";
}
...
sub check_net {
	...
	my $return_str = "";
		...
		#print "NET|$status $return_str $perfdata";
		print "NET|$perfdata";
        }
}
